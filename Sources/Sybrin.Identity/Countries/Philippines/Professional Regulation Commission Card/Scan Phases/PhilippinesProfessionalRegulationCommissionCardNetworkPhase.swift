//
//  PhilippinesProfessionalRegulationCommissionCardNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesProfessionalRegulationCommissionCardNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesProfessionalRegulationCommissionCardModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesProfessionalRegulationCommissionCardModel()
    ...
    func PostAuditData(completion: @escaping(Result<String, IdentityNetworkError>) -> Void) {
        ...
        let urlString = "\(orch_base_url ?? "https://soma.sybrin.com/")" + "api/Authentication/GetAuthToken"
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("DRIVERSLICENSE", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilProfessionalRegulationCommissionCardResponseModel.self, from: data)
                 
                 let model = PhilippinesProfessionalRegulationCommissionCardModel();
                 
                 if let _extracted = json.ExtractedProfessionalRegulationCommissionCardData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedProfessionalRegulationCommissionCardData.self, from: Data(_extracted))
                     
                     model.LastName = extracted.LastName
                     model.FirstName = extracted.FirstName
                     model.DateOfBirth = extracted.DateOfBirth?.toDate()
                     model.ValidUntil = extracted.ValidUntil?.toDate()
                     model.DateIssued = extracted.DateOfIssued?.toDate()
                     model.Profession = extracted.Profession
                     model.MiddleInitialName = extracted.MiddleInitialName
                     model.RegistrationDate = extracted.RegistrationDate?.toDate()
                     model.RegistrationNumber = extracted.RegistrationNumber
