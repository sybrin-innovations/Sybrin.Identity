//
//  PhilippinesDriversLicenseNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesDriversLicenseNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesDriversLicenseModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesDriversLicenseModel()
    ...
    override func ProcessTextRecognition() {
        ...
        self.Model = PhilippinesDriversLicenseModel();
        self.Finished = true
        return;
    ...
    func uploadImageToServer(auth: String) {
        ...
        var mediaImage2: Media?
        
        if let backImage = Constants.Backimage {
            mediaImage2 = Media(withImage: backImage, forKey: "image", name: "BackImage")
        }
       ...
       request.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        ...
        if mediaImage2 != nil {
            mediaList = [mediaImage, mediaImage2!]
        } else {
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilDriversLicenseResponseModel.self, from: data)
                 
                 let model = PhilippinesDriversLicenseModel();
                 if let _extracted = json.ExtractedDriversLicenseData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedDriversLicenseData.self, from: Data(_extracted))
                     ...
                     model.AgencyCode = extracted.AgencyCode
                     model.ExpirationDate = extracted.ExpirationDate?.toDate()
                     model.SerialNumber = extracted.SerialNumber
                     model.LicenseNumber = extracted.LicenseNumber
