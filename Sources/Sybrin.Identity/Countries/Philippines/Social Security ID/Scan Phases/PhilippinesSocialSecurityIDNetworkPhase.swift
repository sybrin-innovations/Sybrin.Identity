//
//  PhilippinesSocialSecurityIDNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesSocialSecurityIDNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesSocialSecurityIDModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesSocialSecurityIDModel()
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("SOCIALSECURITY", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilSocialSecurityResponseModel.self, from: data)
                 
                 let model = PhilippinesSocialSecurityIDModel();
                 
                 if let _extracted = json.ExtractedSocialSecurityData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedSocialSecurityData.self, from: Data(_extracted))
                     
                     model.SocialSecurityNumber = extracted.SocialSecurityNumber
