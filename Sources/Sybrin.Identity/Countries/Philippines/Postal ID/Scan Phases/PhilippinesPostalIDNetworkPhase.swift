...
class PhilippinesPostalIDNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesPostalIDModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesPostalIDModel()
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("POSTALID", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilPostalIDResponseModel.self, from: data)
                 
                 let model = PhilippinesPostalIDModel();
                 if (json.ExtractedPostalIDResponseData?.utf8) != nil {
                     let extracted = try JSONDecoder().decode(ExtractedPostalIDResponseData.self, from: Data())
                     ...
                     model.IssuingPostOffice = extracted.IssuingPostOffice
                     model.CardType = extracted.CardType ?? ""
                     model.PostalReferenceNumber = extracted.PostalReferenceNumber
