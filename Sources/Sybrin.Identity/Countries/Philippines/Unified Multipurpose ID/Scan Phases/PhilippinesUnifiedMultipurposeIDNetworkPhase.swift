//
//  PhilippinesUnifiedMultipurposeIDNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
import AVFoundation
////import MLKit
class PhilippinesUnifiedMultipurposeIDNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesUnifiedMultipurposeIDModel?
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("UNIFIEDMULTIPURPOSE", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilUnifiedMPIDResponseModel.self, from: data)
//                 _ = try JSONSerialization.jsonObject(with: Data(json.ExtractedUnifiedMPIDData!.utf8), options: [])
                 let model = PhilippinesUnifiedMultipurposeIDModel();
                 if ((json.ExtractedUnifiedMPIDData) != nil) {
                     let extracted = try JSONDecoder().decode(ExtractedUnifiedMPIDData.self, from: Data())
                     ...
                     model.CommonReferenceNumber = extracted.CommonReferenceNumber
