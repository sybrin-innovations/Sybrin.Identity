//
//  PhilippinesSeafarerIdentificationRecordBookNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesSeafarerIdentificationRecordBookNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesSeafarerIdentificationRecordBookModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesSeafarerIdentificationRecordBookModel()
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("SEAMANSBOOK", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilSeafarerIdentificationRecordBookResponseModel.self, from: data)
                 
                 
                 let model = PhilippinesSeafarerIdentificationRecordBookModel();
                 
                 if let _extracted = json.ExtractedSeafarerIdentificationRecordData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedSeafarerIdentificationRecordData.self, from: Data(_extracted))
                     
                     model.FullName = extracted.FullName
                     model.EyeColor = extracted.EyeColor
                     model.HairColor = extracted.HairColor
                     ...
                     model.Weight = Float(extracted.Weight ?? "0")
                     model.Height = Float(extracted.Height ?? "0")
                     
                     model.DocumentNumber = extracted.DocumentNumber
                     model.DistinguishingMarks = extracted.DistinguishingMarks
                     model.PlaceOfIssue = extracted.PlaceOfIssue
