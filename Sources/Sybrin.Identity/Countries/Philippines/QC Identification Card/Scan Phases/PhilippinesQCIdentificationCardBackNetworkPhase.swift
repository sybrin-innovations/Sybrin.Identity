//
//  PhilippinesQCIdentificationCardBackNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesQCIdentificationCardBackNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesQCIdentificationCardModel?
        ...
        switch result {
            ...
            case .failure(_):
                DispatchQueue.main.async { [weak self] in
                    ...
                    controller.NotifyUser(NotifyMessage.DetectionFailed(for: (self.PreviousModel as? PhilippinesQCIdentificationCardModel)?.IdentityNumber).stringValue, flashBorderColor: nil, flashOverlayColor: nil)
    ...
    override func ProcessTextRecognition() {
        ...
        self.Model = PhilippinesQCIdentificationCardModel();
        self.Finished = true
        return;
       ...
       session.dataTask(with: request) { (data, response, error) in
          ...
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilQCIdentificationCardResponseModel.self, from: data)
                 
                 let model = PhilippinesQCIdentificationCardModel();
                 
                 if let _extracted = json.ExtractedQCIdentificationCardResponseData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedQCIdentificationCardResponseData.self, from: Data(_extracted))
                     
                     model.QCReferenceNumber = extracted.QCReferenceNumber
                     model.Nationality = extracted.Nationality
                     model.DateOfBirth = extracted.DateOfBirth?.toDate()
                     model.CivilStatus = extracted.CivilStatus
                     model.BloodType = extracted.BloodType
                     model.FullName = extracted.FullName
                     model.CitizenType = extracted.CitizenType
                     model.Address = extracted.Address
                     model.ValidUntil = extracted.ValidUntil?.toDate()
                     model.DisabilityType = extracted.DisabilityType
                     if (extracted.Sex == "F") {
