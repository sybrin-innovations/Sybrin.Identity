//
//  PhilippinesIntegratedBarIDNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class PhilippinesIntegratedBarIDNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: PhilippinesIntegratedBarIDModel?
    ...
    final override func ProcessFrame(buffer: CMSampleBuffer) {
      ...
      if self.Finished {
          let model = PhilippinesIntegratedBarIDModel()
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 ...
                 let json = try JSONDecoder().decode(PhilIntegratedBarIDResponseModel.self, from: data)
                 
                 let model = PhilippinesIntegratedBarIDModel();
                 if let _extracted = json.ExtractedIntegratedBarIDData?.utf8 {
                     let extracted = try JSONDecoder().decode(ExtractedIntegratedBarIDData.self, from: Data(_extracted))
                     
                     model.FullName = extracted.FullName
                     model.IntegratedBarPhilippinesChapter = extracted.IntegratedBarPhilippinesChapter
                     model.RollOfAttorneysNumber = extracted.RollOfAttorneysNumber
