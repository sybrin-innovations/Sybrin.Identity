...
struct MalawiPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Malawi.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: MalawiPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToMalawiPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToMalawiPassportModel(_ td3Model: TD3Model) -> MalawiPassportModel? {
        let model = MalawiPassportModel()
        ...
        model.PersonalNumber = td3Model.OptionalData
        model.PersonalNumberCheckDigit = td3Model.OptionalDataCheckDigit
