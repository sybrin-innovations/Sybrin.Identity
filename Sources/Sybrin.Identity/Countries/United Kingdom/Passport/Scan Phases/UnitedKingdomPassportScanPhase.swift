//
//  UnitedKingdomPassportScanPhase.swift
//  Sybrin.iOS.Identity
//
...
final class UnitedKingdomPassportScanPhase: ScanPhase<DocumentModel> {
    
    // MARK: Private Properties
    private final var Face: Face?
    private final var LastFrame: CMSampleBuffer?
    private final var Parser = UnitedKingdomPassportParser()
    ...
    final override func PostProcessing(for result: Result<Bool, ScanFailReason>, done: @escaping () -> Void) {
        ...
        switch result {
            case .success(_):
                guard let model = Model as? UnitedKingdomPassportModel else { return }
