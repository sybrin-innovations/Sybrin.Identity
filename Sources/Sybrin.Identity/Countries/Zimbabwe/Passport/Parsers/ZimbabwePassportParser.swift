...
struct ZimbabwePassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Zimbabwe.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: ZimbabwePassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToZimbabwePassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToZimbabwePassportModel(_ td3Model: TD3Model) -> ZimbabwePassportModel? {
        let model = ZimbabwePassportModel()
