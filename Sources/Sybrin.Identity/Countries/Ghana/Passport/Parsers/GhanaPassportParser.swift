...
struct GhanaPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Ghana.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: GhanaPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToGhanaPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToGhanaPassportModel(_ td3Model: TD3Model) -> GhanaPassportModel? {
        let model = GhanaPassportModel()
