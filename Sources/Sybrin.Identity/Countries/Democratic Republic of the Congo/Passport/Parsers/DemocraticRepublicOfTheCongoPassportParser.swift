//
//  DemocraticRepublicOfTheCongoPassportParser.swift
//  Sybrin.iOS.Identity
//
...
struct DemocraticRepublicOfTheCongoPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.DemocraticRepublicOfTheCongo.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: DemocraticRepublicOfTheCongoPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToDemocraticRepublicOfTheCongoPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToDemocraticRepublicOfTheCongoPassportModel(_ td3Model: TD3Model) -> DemocraticRepublicOfTheCongoPassportModel? {
        let model = DemocraticRepublicOfTheCongoPassportModel()
