...
final class UgandaPassportScanPhase: ScanPhase<DocumentModel> {
    
    // MARK: Private Properties
    private final var Face: Face?
    private final var LastFrame: CMSampleBuffer?
    private final var Parser = UgandaPassportParser()
    ...
    final override func PostProcessing(for result: Result<Bool, ScanFailReason>, done: @escaping () -> Void) {
        ...
        switch result {
            case .success(_):
                guard let model = Model as? UgandaPassportModel else { return }
