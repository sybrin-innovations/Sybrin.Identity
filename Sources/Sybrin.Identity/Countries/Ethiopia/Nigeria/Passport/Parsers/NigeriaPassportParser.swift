...
struct NigeriaPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Nigeria.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: NigeriaPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToNigeriaPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToNigeriaPassportModel(_ td3Model: TD3Model) -> NigeriaPassportModel? {
        let model = NigeriaPassportModel()
