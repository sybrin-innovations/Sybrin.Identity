//
//  EthiopiaPassportParser.swift
//  Sybrin.iOS.Identity
//
//  Created by Armand Riley on 2021/08/17.
//  Copyright © 2021 Sybrin Systems. All rights reserved.
//
//import MLKit

struct EthiopiaPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Ethiopia.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: EthiopiaPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToEthiopiaPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToEthiopiaPassportModel(_ td3Model: TD3Model) -> EthiopiaPassportModel? {
        let model = EthiopiaPassportModel()
