//
//  ZambiaPassportParser.swift
//  Sybrin.iOS.Identity
//
//  Created by Armand Riley on 2021/08/19.
//  Copyright © 2021 Sybrin Systems. All rights reserved.
//
import MLKit

struct ZambiaPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Zambia.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: ZambiaPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToZambiaPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToZambiaPassportModel(_ td3Model: TD3Model) -> ZambiaPassportModel? {
        let model = ZambiaPassportModel()
