//
//  BangladeshPassportParser.swift
//  Sybrin.iOS.Identity
//
...
struct BangladeshPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Bangladesh.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: BangladeshPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToBangladeshPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToBangladeshPassportModel(_ td3Model: TD3Model) -> BangladeshPassportModel? {
        let model = BangladeshPassportModel()
