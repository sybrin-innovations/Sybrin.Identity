//
//  SouthAfricaGreenBookNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class SouthAfricaGreenBookNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: SouthAfricaGreenBookModel?
    ...
    func uploadImageToServer(auth: String) {
       ...
       var request = URLRequest(url: url)
        ...
        request.setValue("GREENBOOK", forHTTPHeaderField: "DocumentType")
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 let json = try JSONDecoder().decode(SAGreenBookResponseModel.self, from: data)
                 
                 let model = SouthAfricaGreenBookModel();
                 
                 if  let _extracted = json.ExtractedSAGreenBookData?.utf8 {
                     
                     let extracted = try JSONDecoder().decode(ExtractedSAGreenBookData.self, from: Data(_extracted))

                     model.IdentityNumber = extracted.IdentityNumber
                     model.Surname = extracted.Surname
                     model.FirstNames = extracted.FirstNames
...
//                     model.BookType = extracted.BookType
