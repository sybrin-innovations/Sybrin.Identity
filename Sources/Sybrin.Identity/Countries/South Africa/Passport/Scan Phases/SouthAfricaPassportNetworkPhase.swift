//
//  SouthAfricaPassportNetworkPhase.swift
//  Sybrin.iOS.Identity
//
...
class SouthAfricaPassportNetworkPhase: ScanPhase<DocumentModel> {
    ...
    private var nuModel: SouthAfricaPassportModel?
    ...
    func uploadImageToServer(auth: String) {
       ...
       session.dataTask(with: request) { (data, response, error) in
          if let data = data {
             do {
                 let json = try JSONDecoder().decode(SAPassportResponseModel.self, from: data)
                 
                 let model = SouthAfricaPassportModel();
                 
                 if  let _extracted = json.ExtractedSAPassportData?.utf8 {
                     
                     let extracted = try JSONDecoder().decode(ExtractedSAPassportData.self, from: Data(_extracted))
