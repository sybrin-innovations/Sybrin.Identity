...
struct MauritiusPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Mauritius.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: MauritiusPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToMauritiusPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToMauritiusPassportModel(_ td3Model: TD3Model) -> MauritiusPassportModel? {
        let model = MauritiusPassportModel()
