...
struct LesothoPassportParser {
    
    // MARK: Private Properties
    // Parser
    private let Parser = TraditionalTD3Parser(issuingCountryCode: Country.Lesotho.passportCountryCode)
    ...
    mutating func Parse(from result: Text, success: @escaping (_ model: LesothoPassportModel) -> Void) {
        ...
        if Parser.Line1Parsed && Parser.Line2Parsed, let model = TD3ModelToLesothoPassportModel(Parser.Model) {
            success(model)
            Reset()
        }
    ...
    private func TD3ModelToLesothoPassportModel(_ td3Model: TD3Model) -> LesothoPassportModel? {
        let model = LesothoPassportModel()
