...
class TraditionalTD2Parser {
    
    // MARK: Internal Properties
    var Line1: String?
    var Line2: String?
    var Line1Regex: NSRegularExpression { get { return RegexConstants.TD2_MRZ_Line1 } }
    var Line2Regex: NSRegularExpression { get { return RegexConstants.TD2_MRZ_Line2 } }
    var Model: TD2Model = TD2Model()
    ...
    func ParseLine1(from line: String) {
        ...
        guard line.count == 36 else { "Validation Failed: Line 1 Length".log(.ProtectedWarning); return }
    ...
    func ParseLine2(from line: String) {
        ...
        guard line.count == 36 else { "Validation Failed: Line 2 Length".log(.ProtectedWarning); return }
        ...
        Model.OptionalData = line.substring(with: 28..<35)
        
        let optionalData = Model.OptionalData ?? ""
        
        Model.CompositeCheckDigit = Int(line.substring(with: 35..<36))
        ...
        guard CheckDigitValidator.GetCheckDigitUsingMRZAlgorithm(for: "\(documentNumber)\(documentNumberCheckDigit)\(dateOfBirth)\(dateOfBirthCheckDigit)\(dateOfExpiry)\(dateOfExpiryCheckDigit)\(optionalData)") == compositeCheckDigit else { "Validation Failed: Composite Check Digit".log(.ProtectedWarning); return }
